...
def calculate_energy_savings(
    decommissioned_hosts: int
) -> float:
    """Calculates datacenter environmental and maintenance cost metrics.

    Args:
        decommissioned_hosts (int):
            Number of retired physical hosts

    Returns:
        float:
            Estimated cost saving percentage
    """
    ...
    logging.info(
        f"Consolidation metrics parsed. "
        f"Estimated savings calculated at: {savings_ratio}%"
    )
